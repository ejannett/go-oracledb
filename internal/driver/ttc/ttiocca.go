...
func (m *tTIOcca) MarshalTo(ctx context.Context, engine driverCommon.Marshaller) error {
	...
	if err := m.headerMarshaller.MarshalTo(ctx, engine); err != nil {
		...
		return common.NewOracleError(oracleErrors.FailMarshal, err, "OCCA header")
	}
	...
	if err := engine.MarshalPTR(ctx); err != nil {
		...
		return common.NewOracleError(oracleErrors.FailMarshal, err, "OCCA cursorId")
	}
	...
	if err := engine.MarshalUB4(ctx, offset); err != nil {
		...
		return common.NewOracleError(oracleErrors.FailMarshal, err, "OCCA offset")
	}
	...
	for i := 0; i < int(offset); i++ {
		if err := engine.MarshalUB4(ctx, m.cursorIDs[i]); err != nil {
			...
			return common.NewOracleError(oracleErrors.FailMarshal, err, "OCCA cursor ID")
		}
	}
